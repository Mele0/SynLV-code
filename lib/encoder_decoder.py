...
class GRU_unit(nn.Module):
	def __init__(self, latent_dim, input_dim, update_gate = None, reset_gate = None, new_state_net = None, n_units = 100, gru_aug = False, device = torch.device("cpu")):
...
class Encoder_z0_ODE_RNN(nn.Module):
	...
	def forward(self, data, time_steps, run_backwards = True, save_info = False):
		...
		if len(time_steps) == 1:
			...
			last_yi, last_yi_std, latent_ys, latent_ys_std, extra_info = self.run_odernn(data, time_steps, run_backwards = run_backwards, save_info = save_info)
