...
class LatentODE(nn.Module):
	...
	def get_reconstruction_traj(self, results, batch_dict, filename_suffix=None, feat_names=None,
		...
		if high_prop_feats_idx is not None:
			...
			observed_data = batch_dict['data_to_predict']
			pred_data = results['pred_y'][0]
	...
	def compute_all_losses(self, batch_dict, n_latent_traj=1, kl_coef=1., surv_est=None,
		...
		if n_latent_traj > 1:
			...
			if self.check_extrapolation:
				results["pred_y_extr"] = pred_y_mean_extr
		else:
			results["pred_y"] = pred_y
			if self.check_extrapolation:
				results["pred_y_extr"] = pred_y_extr
		return results

	def get_reconstruction_survival(self, time_steps_to_predict, truth, truth_time_steps, end_of_obs_idx, mask=None, n_latent_traj=1, run_backwards=True, eps=1e-7, get_multiple_traj=False, test_batch_size=100,
									get_latent_hazard=False, temporal_encoding=False, latent_harzard_time_window=365):
		...
		if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
			...
			means_z0 = first_point_mu
			sigma_z0 = first_point_std
	...
	def sample_traj_from_prior(self, time_steps_to_predict, n_traj_samples = 1):
		...
		sol_y = self.diffeq_solver.sample_traj_from_prior(starting_point_enc_aug, time_steps_to_predict, n_traj_samples = 3)
		if self.use_poisson_proc:
