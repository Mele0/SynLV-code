...
class LatentODESub(LatentODE):  
	def __init__(self, latent_dim=20, rec_dim=40, rec_layers=3, gen_layers=3, units_ode=50, units_gru=50, input_dim=20, reconstr_dim=20, device=None, gru_aug=False, attention_aug=False, attn_num_heads=4, n_events=1, temporal_encoding=False, num_layer_hazard_dec=2, mult_event_units=5):
		...
		ode_func_net = utils.create_net(latent_dim, latent_dim, n_layers=gen_layers, n_units=units_ode, nonlinear=nn.Tanh)
		gen_ode_func = ODEFunc(input_dim=input_dim, latent_dim=latent_dim, ode_func_net=ode_func_net, device=device).to(device)

		enc_input_dim = int(input_dim) * 2
		ode_func_net = utils.create_net(rec_dim, rec_dim, n_layers=rec_layers, n_units=units_ode, nonlinear=nn.Tanh)
		...
		super().__init__(input_dim = input_dim, latent_dim = latent_dim, encoder_z0 = encoder_z0, decoder = decoder, decoder_surv = decoder_surv if n_events ==1 else (decoder_surv_1, decoder_surv_2), diffeq_solver = diffeq_solver, z0_prior = z0_prior, device = device, obsrv_std = obsrv_std, gru_aug = gru_aug, n_gru_units = units_gru)
	...
	def fit(self, data_train, data_valid, data_info_dic, device=None, n_epochs=30, batch_size=50,
			...
			feat_reconstr=None, check_extrapolation=False, wait_until_full_surv_loss=15, dataset='general', exclude_samples_wo_pred_window=True, random_seed=0):
		self.check_extrapolation = check_extrapolation
		self.dataset = dataset
		self.random_seed = random_seed
		...
		(data_obj, self.min_max_tuple,
			self.max_obs_time, self.event_to_event_weight_dict) = utils.get_data_obj_merged(data_train, data_valid, data_info_dic, device=self.device, feat_reconstr=feat_reconstr, max_pred_window=max_pred_window, n_events=self.n_events, random_seed=self.random_seed, param_dics=param_dics, exclude_samples_wo_pred_window=exclude_samples_wo_pred_window,)
